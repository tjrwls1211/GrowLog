...
describe('DELETE /api/reports/[reportId]', () => {
  it('로그인한 사용자가 자신의 리포트를 삭제할 수 있어야 한다', async () => {
    const testUser = await prisma.user.create({
      data: {
        email: 'test@example.com',
        name: '테스트 유저',
    ...
    const response = await DELETE(request, { params: { reportId: String(report.id) } })
    const data = await response.json()

    expect(response.status).toBe(200)
    expect(data).toHaveProperty('message')
    expect(data.message).toBe('리포트가 삭제되었습니다.')
  ...
  it('존재하지 않는 리포트 ID는 404를 반환해야 한다', async () => {
    ...
    const response = await DELETE(request, { params: { reportId: '999' } })
    const data = await response.json()
  ...
  it('다른 사용자의 리포트는 삭제할 수 없고 403을 반환해야 한다', async () => {
    const testUser = await prisma.user.create({
      data: {
        email: 'test@example.com',
        name: '테스트 유저',
    ...
    const stillExists = await prisma.report.findUnique({
      where: { id: report.id },
    })
  ...
  it('유효하지 않은 리포트 ID는 400을 반환해야 한다', async () => {
    ...
    const response = await DELETE(request, { params: { reportId: 'invalid' } })
    const data = await response.json()
  ...
  it('로그인하지 않은 경우 401 에러를 반환해야 한다', async () => {
    ...
    const response = await DELETE(request, { params: { reportId: '1' } })
    const data = await response.json()
